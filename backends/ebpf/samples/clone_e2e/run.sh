...
clang -O2 -target bpf -c ce2e-kern.c -o ce2e-kern.o
if [ $? -ne 0 ]; then
...
sudo nsenter --net=/var/run/netns/switch tc filter add dev eth0 egress bpf da obj ce2e-kern.o sec tc-egress verbose
#sudo nsenter --net=/var/run/netns/switch tc filter add dev eth1 egress bpf da obj ce2e-kern.o sec tc-egress verbose
sudo nsenter --net=/var/run/netns/switch tc filter add dev eth2 egress bpf da obj ce2e-kern.o sec tc-egress verbose

echo "Adding BPF prog to eth0 of switch"
sudo nsenter --net=/var/run/netns/switch tc filter add dev eth0 ingress bpf da obj ce2e-kern.o sec pktclone verbose