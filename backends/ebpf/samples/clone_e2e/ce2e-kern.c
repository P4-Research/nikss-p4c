
#include <linux/bpf.h>
#include <linux/pkt_cls.h>
#include <bpf/bpf_helpers.h>
#include <stddef.h>
#include <stdbool.h>
#include <stdint.h>

#define PIN_GLOBAL_NS		2

#define SEC(NAME) __attribute__((section(NAME), used))

struct bpf_elf_map {
    /*
     * The various BPF MAP types supported (see enum bpf_map_type)
     * https://elixir.bootlin.com/linux/latest/source/include/uapi/linux/bpf.h
     */
    __u32 type;
    __u32 size_key;
    __u32 size_value;
    __u32 max_elem;
    /*
     * Various flags you can place such as `BPF_F_NO_COMMON_LRU`
     */
    __u32 flags;
    __u32 id;
    /*
     * Pinning is how the map are shared across process boundary.
     * Cillium has a good explanation of them: http://docs.cilium.io/en/v1.3/bpf/#llvm
     * PIN_GLOBAL_NS - will get pinned to `/sys/fs/bpf/tc/globals/${variable-name}`
     * PIN_OBJECT_NS - will get pinned to a directory that is unique to this object
     * PIN_NONE - the map is not placed into the BPF file system as a node,
                   and as a result will not be accessible from user space
     */
    __u32 pinning;

    __u32 inner_id;
    __u32 inner_idx;
};

#define bpf_debug_printk(fmt, ...)                               \
  ({                                                             \
      char ____fmt[] = fmt;                                      \
      bpf_trace_printk(____fmt, sizeof(____fmt), ##__VA_ARGS__); \
  })

/***
 * PACKET CLONING (CI2E) EXAMPLE START
 *
 * For PSA, we need to have the BPF map storing a list of (egress_port, instance) pairs. To implement such a construct
 * in the eBPF program we can:
 * 1) Use map-in-map mechanism, but it requires to create 1 ("master" table returning "mcast" table id based on clone_session_id)
 *    + N (number of "mcast" tables, depends on number of clone_session_id's configured) number of maps.
 *    This is perhaps a more mature solution, which can be used as a target solution. It also requires to dynamically create BPF maps from userspace.
 * 2) Use a single map storing only one, but large element (array of struct egress_pair).
 *    The number of elements is predefined (sets the maximum number of clone_session_id's).
 *    Each element in the array is zero-initialized.
 *    Adding a new member of clone_session_id requires to initialize the element's egress_port != 0.
 */

#define MAX_PORTS 256
#define MAX_INSTANCES 16

struct egress_pair {
    uint32_t egress_port;
    uint16_t instance;
    uint8_t  class_of_service;
    uint8_t  truncate;
    uint16_t packet_length_bytes;
};

struct bpf_elf_map SEC("maps") clone_session_pairs = {
        .type = BPF_MAP_TYPE_HASH,
        .size_key = sizeof(__u32),
        .size_value = sizeof(struct egress_pair),
        .max_elem = 2,
        .pinning = 2,
        .id		= 1,
        .inner_idx	= 1,
};

struct bpf_elf_map SEC("maps") clone_session_tbl = {
        .type = BPF_MAP_TYPE_HASH_OF_MAPS,
        .size_key = sizeof(__u32),
        .size_value = sizeof(__u32),
        .flags = 0,
        .pinning = 2,
        .inner_id = 1,
        .max_elem = 32,
};

enum PacketPath_t {
    NORMAL,     /// Packet received by ingress that is none of the cases below.
    NORMAL_UNICAST,   /// Normal packet received by egress which is unicast
    NORMAL_MULTICAST, /// Normal packet received by egress which is multicast
    CLONE_I2E,  /// Packet created via a clone operation in ingress,
    /// destined for egress
    CLONE_E2E,  /// Packet created via a clone operation in egress,
    /// destined for egress
    RESUBMIT,   /// Packet arrival is the result of a resubmit operation
    RECIRCULATE /// Packet arrival is the result of a recirculate operation
};

struct psa_standard_metadata {
    enum PacketPath_t packet_path;
    uint32_t clone_session_id;
};

/*
 * This eBPF program implements PoC of simple packet clone scenario (CI2E) for PSA.
 * In the P4 (PSA) code, packet cloning is realized by setting metadata fields.
 * The sample "clone" action looks as follows:
 *
 * action do_clone (CloneSessionId_t session_id) {
 *     ostd.clone = true;
 *     ostd.clone_session_id = session_id;
 * }
 */
SEC("pktclone")
int pkt_clone(struct __sk_buff *skb)
{
    /* Metadata, which comes to TC Ingress from the Ingress pipeline contains following values. */
    struct psa_ingress_metadata_t {
        bool clone;
        uint32_t clone_session_id;
    } meta = {
            .clone = true,
            .clone_session_id = 1,
    };

    // FIXME: temporarily, we use skb->data to store metadata. We initialize it with packet_path = NORMAL.
    struct psa_standard_metadata *std_meta = (struct psa_standard_metadata *) skb->data;
    if (std_meta + 1 > skb->data_end) {
        bpf_debug_printk("[INGRESS] Size exceeded\n");
        return TC_ACT_SHOT;
    }
    std_meta->packet_path = NORMAL;

    bpf_debug_printk("[INGRESS] PSA standard metadata: packet_path=%d", std_meta->packet_path);

    // This will be always true, but it shows generic condition to check if packet should be cloned by "Traffic Manager"
    if (meta.clone) {
        struct bpf_elf_map *inner_map;
        bpf_debug_printk("[INGRESS] Looking for clone_session_id = %d\n", meta.clone_session_id);
        inner_map = bpf_map_lookup_elem(&clone_session_tbl, &meta.clone_session_id);
        if (!inner_map) {
            bpf_debug_printk("[INGRESS] Unsupported ostd.clone_session_id value (bpf: inner map not found)\n");
            return TC_ACT_SHOT;
        }

        bpf_debug_printk("[INGRESS] Clone Session with ID %d found.\n", meta.clone_session_id);

        for (int i = 0; i < MAX_PORTS * MAX_INSTANCES; i++) {
            int idx = i;
            struct egress_pair *pair = (struct egress_pair *) bpf_map_lookup_elem(inner_map, &idx);
            if (pair == NULL) {
                bpf_debug_printk("[INGRESS] No more clone session entries found, aborting\n");
                // we don't have more pairs in the map, continue..
                return TC_ACT_SHOT;
            }

            bpf_debug_printk("[INGRESS] Clone session entry found. Clone session parameters: class_of_service=%d\n",
                             pair->class_of_service);
            bpf_debug_printk("[INGRESS] Redirecting to port %d\n", pair->egress_port);
            bpf_clone_redirect(skb, pair->egress_port, 0);
        }
        return TC_ACT_SHOT;
    }

    return TC_ACT_SHOT;
}

/***
 * PACKET CLONING (CI2E) EXAMPLE END
 */

SEC("tc-egress")
int tc_egress(struct __sk_buff *skb)
{
    char fmt2[] = "[EGRESS] Handling packet in the TC egress. Egress port = %d";
    bpf_trace_printk(fmt2, sizeof(fmt2), skb->ifindex);

    struct psa_standard_metadata *std_meta = (struct psa_standard_metadata *) skb->data;
    if (std_meta + 1 > skb->data_end) {
        bpf_debug_printk("[EGRESS] Size exceeded");
        return TC_ACT_SHOT;
    }

    bpf_debug_printk("[EGRESS] PSA standard metadata: packet_path=%d", std_meta->packet_path);

    if (std_meta->packet_path == CLONE_E2E) {
        bpf_debug_printk("[EGRESS] Packet is cloned E2E, going out to port %d", skb->ifindex);
        return TC_ACT_OK;
    }

    bool clone = false;
    if (skb->ifindex == 4) {
        // perform CE2E only for packets directed to port 3
        clone = true;
    }

    if (clone) {
        std_meta->clone_session_id = 5;
        /*
         * Note! We could use bpf_clone_redirect() with BPF_F_INGRESS flag to pass packet back to "Traffic Manager"
         * and select output port based on `clone_session_id`. However, it would introduce an additional cost. Thus,
         * we can use BPF map, which is shared between TC programs to determine output port in the Egress.
         */
        struct bpf_elf_map *inner_map;
        bpf_debug_printk("[EGRESS] Looking for clone_session_id = %d", std_meta->clone_session_id);
        inner_map = bpf_map_lookup_elem(&clone_session_tbl, &std_meta->clone_session_id);
        if (!inner_map) {
            bpf_debug_printk("[EGRESS] Unsupported ostd.clone_session_id value (bpf: inner map not found)\n");
            return TC_ACT_SHOT;
        }

        bpf_debug_printk("[EGRESS] Clone Session with ID %d found.", std_meta->clone_session_id);
        std_meta->packet_path = CLONE_E2E;
        for (int i = 0; i < MAX_PORTS * MAX_INSTANCES; i++) {
            int idx = i;
            struct egress_pair *pair = (struct egress_pair *) bpf_map_lookup_elem(inner_map, &idx);
            if (pair == NULL) {
                bpf_debug_printk("[EGRESS] No more clone session entries found, aborting\n");
                // we don't have more pairs in the map, continue..
                return TC_ACT_SHOT;
            }

            bpf_debug_printk("[EGRESS] Clone session entry found. Clone session parameters: class_of_service=%d\n",
                             pair->class_of_service);
            bpf_debug_printk("[EGRESS] Redirecting to port %d", pair->egress_port);

            bpf_clone_redirect(skb, pair->egress_port, 0);
        }

        return TC_ACT_SHOT;
    }

    return TC_ACT_OK;
}

static char _license[] SEC("license") = "GPL";