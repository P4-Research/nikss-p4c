...
{
    int error = 0;
    int inner_map_fd = bpf_create_map(BPF_MAP_TYPE_HASH, sizeof(__u32), sizeof(struct clone_session_entry),
                                      MAX_MEMBERS, 0);
    if (inner_map_fd < 0) {
...
int clone_session_add_member(uint32_t clone_session_id, struct clone_session_entry pair)
{
    char pinned_file[256];
    snprintf(pinned_file, sizeof(pinned_file), "%s/%s", TC_GLOBAL_NS,
             "clone_session_tbl");
 ...
 * $ ./main session-add-member 1 2 1 1
...
int main(int argc, char **argv) {
    ...
    if (strcmp(argv[1], "session-create") == 0) {
        ...
        struct clone_session_entry pair = {
