...
/* extract(headers.outer_udp)*/
    ...
    hdr_offset.udp = BYTES(ebpf_packetOffsetInBits);
...
//    headers.ipv4.ebpf_valid = 1;
                ...
                if (value != NULL) {
                    ...
                    switch (value->action) {
                        case ACT_INGRESS_VXLAN_ENCAP:
                        {
                            ...
                            headers.outer_udp.length = headers.ipv4.total_len + 16;
    ...
    }
    {
        ...
        if (hdr_valid.outer_ethernet_valid) {
            ...
            __builtin_memcpy(pkt + BYTES(ebpf_packetOffsetInBits) + 6, (void *) &(headers.outer_ethernet.src_addr), 6);
            ebpf_packetOffsetInBits += 48;

            headers.outer_ethernet.ether_type = bpf_htons(headers.outer_ethernet.ether_type);
            __builtin_memcpy(pkt + BYTES(ebpf_packetOffsetInBits) + 12, (void *) &(headers.outer_ethernet.ether_type), 2);
            ebpf_packetOffsetInBits += 16;
        ...
        if (hdr_valid.vxlan_valid) {
            ...
            __builtin_memcpy(pkt + BYTES(ebpf_packetOffsetInBits), (void *) &(headers.vxlan.reserved), 1);
            ebpf_packetOffsetInBits += 8;
        ...
        if (hdr_valid.ipv4_valid) {
            ...
            __builtin_memcpy(pkt + BYTES(ebpf_packetOffsetInBits), (void *) &(tmp_header.ipv4), 20);
