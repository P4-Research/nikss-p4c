...
struct udp_t {
    ...
    u16 length; /* bit<16> */
    u16 checksum; /* bit<16> */
    //u8 ebpf_valid; -> do pomocniczej struktury
};
struct vxlan_t {
    u8 *flags; /* bit<8> */
    u32 *reserved; /* bit<24> */
...
// zmienna pomocnicza ebpf valid dla kazdego nagłówka

// zmienna pomocnicza offset dla kazdego nagłówka
...
/* extract(headers->outer_udp)*/
        ...
        headers->outer_udp = (struct struct udp_t *)(pkt + BYTES(ebpf_packetOffsetInBits));
        //headers->outer_udp.src_port = (u16*)&pkt + BYTES(ebpf_packetOffsetInBits);
        //headers->outer_udp.src_port = (u16)((load_half(pkt, BYTES(ebpf_packetOffsetInBits))));
        //ebpf_packetOffsetInBits += 16;
        //headers->outer_udp.dst_port = (u16*)&pkt + BYTES(ebpf_packetOffsetInBits);
        //headers->outer_udp.dst_port = (u16)((load_half(pkt, BYTES(ebpf_packetOffsetInBits))));
        //ebpf_packetOffsetInBits += 16;
        //headers->outer_udp.length = (u16*)&pkt + BYTES(ebpf_packetOffsetInBits);
        //headers->outer_udp.length = (u16)((load_half(pkt, BYTES(ebpf_packetOffsetInBits))));
        // += 16;
        //headers->outer_udp.checksum = (u16*)&pkt + BYTES(ebpf_packetOffsetInBits);
...
headers->outer_ipv4 = headers->ipv4;/* extract(headers->ipv4)*/
    ...
    struct headers_t tmp_header;
    struct headers_t * ptr = &(tmp_header);
    ...
    accept:
    {
        ...
        {
if (            ptr->vxlan.ebpf_valid) {
    *ptr->ethernet.dst_addr = *ptr->outer_ethernet.dst_addr;            }

                        {
                /* construct key */
                struct ingress_vxlan_key key = {};
                key.field0 = *ptr->ethernet.dst_addr;
                /* value */
                ...
                if (value != NULL) {
                    ...
                    switch (value->action) {
                        case INGRESS_VXLAN_ACT_INGRESS_VXLAN_ENCAP: 
                            {
                                ptr->outer_ethernet.ebpf_valid = true;
                                *ptr->outer_ethernet.src_addr = value->u.ingress_vxlan_encap.ethernet_src_addr;
                                *ptr->outer_ethernet.dst_addr = value->u.ingress_vxlan_encap.ethernet_dst_addr;
                                *ptr->outer_ethernet.ether_type = 2048;
                                                                ptr->outer_ipv4.ebpf_valid = true;
                                *ptr->outer_ipv4.ver_ihl = 69;
                                *ptr->outer_ipv4.diffserv = 0;
                                *ptr->outer_ipv4.total_len = *headers->ipv4.total_len + 50;
                                *ptr->outer_ipv4.identification = 5395;
                                *ptr->outer_ipv4.flags_offset = 0;
                                *ptr->outer_ipv4.ttl = 64;
                                *ptr->outer_ipv4.protocol = 17;
                                *ptr->outer_ipv4.src_addr = value->u.ingress_vxlan_encap.ipv4_src_addr;
                                *ptr->outer_ipv4.dst_addr = value->u.ingress_vxlan_encap.ipv4_dst_addr;
                                                                ptr->outer_udp.ebpf_valid = true;
                                *ptr->outer_udp.src_port = 15221;
                                *ptr->outer_udp.dst_port = 4789;
                                *ptr->outer_udp.length = *headers->ipv4.total_len + 30;
                                                                ptr->vxlan.ebpf_valid = true;
                                *ptr->vxlan.flags = 0;
                                *ptr->vxlan.reserved = 0;
                                *ptr->vxlan.vni = value->u.ingress_vxlan_encap.vxlan_vni;
                                *ptr->vxlan.reserved2 = 0;
                                ostd->drop = false;
                        ...
                        case INGRESS_VXLAN_ACT_INGRESS_VXLAN_DECAP: 
                            {
                                ptr->outer_ethernet.ebpf_valid = false;
                                ptr->outer_ipv4.ebpf_valid = false;
                                ptr->outer_udp.ebpf_valid = false;
                                ptr->vxlan.ebpf_valid = false;
                                ostd->drop = false;
...
// ################################################### DEPARSER #########################################################################################################
        ...
        if (ptr->outer_ethernet.ebpf_valid) {
            if (ebpf_packetEnd < pkt + BYTES(ebpf_packetOffsetInBits + 112)) {
                return TC_ACT_SHOT;
            }

            *ptr->outer_ethernet.dst_addr = htonll(*ptr->outer_ethernet.dst_addr << 16);
            ebpf_byte = ((char *) (&ptr->outer_ethernet.dst_addr))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->outer_ethernet.dst_addr))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->outer_ethernet.dst_addr))[2];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 2, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->outer_ethernet.dst_addr))[3];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 3, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->outer_ethernet.dst_addr))[4];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 4, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->outer_ethernet.dst_addr))[5];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 5, (ebpf_byte));
            ebpf_packetOffsetInBits += 48;

            *ptr->outer_ethernet.src_addr = htonll(*ptr->outer_ethernet.src_addr << 16);
            ebpf_byte = ((char *) (&ptr->outer_ethernet.src_addr))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->outer_ethernet.src_addr))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->outer_ethernet.src_addr))[2];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 2, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->outer_ethernet.src_addr))[3];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 3, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->outer_ethernet.src_addr))[4];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 4, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->outer_ethernet.src_addr))[5];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 5, (ebpf_byte));
            ebpf_packetOffsetInBits += 48;

            *ptr->outer_ethernet.ether_type = bpf_htons(*ptr->outer_ethernet.ether_type);
            ebpf_byte = ((char *) (&ptr->outer_ethernet.ether_type))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->outer_ethernet.ether_type))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_packetOffsetInBits += 16;

        }
        if (ptr->outer_ipv4.ebpf_valid) {
            if (ebpf_packetEnd < pkt + BYTES(ebpf_packetOffsetInBits + 160)) {
                return TC_ACT_SHOT;
            }

            ebpf_byte = ((char *) (&ptr->outer_ipv4.ver_ihl))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_packetOffsetInBits += 8;

            ebpf_byte = ((char *) (&ptr->outer_ipv4.diffserv))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_packetOffsetInBits += 8;

            *ptr->outer_ipv4.total_len = bpf_htons(*ptr->outer_ipv4.total_len);
            ebpf_byte = ((char *) (&ptr->outer_ipv4.total_len))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->outer_ipv4.total_len))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_packetOffsetInBits += 16;

            *ptr->outer_ipv4.identification = bpf_htons(*ptr->outer_ipv4.identification);
            ebpf_byte = ((char *) (&ptr->outer_ipv4.identification))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->outer_ipv4.identification))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_packetOffsetInBits += 16;

            *ptr->outer_ipv4.flags_offset = bpf_htons(*ptr->outer_ipv4.flags_offset);
            ebpf_byte = ((char *) (&ptr->outer_ipv4.flags_offset))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->outer_ipv4.flags_offset))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_packetOffsetInBits += 16;

            ebpf_byte = ((char *) (&ptr->outer_ipv4.ttl))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_packetOffsetInBits += 8;

            ebpf_byte = ((char *) (&ptr->outer_ipv4.protocol))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_packetOffsetInBits += 8;

            *ptr->outer_ipv4.hdr_checksum = bpf_htons(*ptr->outer_ipv4.hdr_checksum);
            ebpf_byte = ((char *) (&ptr->outer_ipv4.hdr_checksum))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->outer_ipv4.hdr_checksum))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_packetOffsetInBits += 16;

            *ptr->outer_ipv4.src_addr = htonl(*ptr->outer_ipv4.src_addr);
            ebpf_byte = ((char *) (&ptr->outer_ipv4.src_addr))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->outer_ipv4.src_addr))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->outer_ipv4.src_addr))[2];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 2, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->outer_ipv4.src_addr))[3];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 3, (ebpf_byte));
            ebpf_packetOffsetInBits += 32;

            *ptr->outer_ipv4.dst_addr = htonl(*ptr->outer_ipv4.dst_addr);
            ebpf_byte = ((char *) (&ptr->outer_ipv4.dst_addr))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->outer_ipv4.dst_addr))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->outer_ipv4.dst_addr))[2];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 2, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->outer_ipv4.dst_addr))[3];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 3, (ebpf_byte));
            ebpf_packetOffsetInBits += 32;

        }
        if (ptr->outer_udp.ebpf_valid) {
            if (ebpf_packetEnd < pkt + BYTES(ebpf_packetOffsetInBits + 64)) {
                return TC_ACT_SHOT;
            }

            *ptr->outer_udp.src_port = bpf_htons(*ptr->outer_udp.src_port);
            ebpf_byte = ((char *) (&ptr->outer_udp.src_port))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->outer_udp.src_port))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_packetOffsetInBits += 16;

            *ptr->outer_udp.dst_port = bpf_htons(*ptr->outer_udp.dst_port);
            ebpf_byte = ((char *) (&ptr->outer_udp.dst_port))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->outer_udp.dst_port))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_packetOffsetInBits += 16;

            *ptr->outer_udp.length = bpf_htons(*ptr->outer_udp.length);
            ebpf_byte = ((char *) (&ptr->outer_udp.length))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->outer_udp.length))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_packetOffsetInBits += 16;

            *ptr->outer_udp.checksum = bpf_htons(*ptr->outer_udp.checksum);
            ebpf_byte = ((char *) (&ptr->outer_udp.checksum))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->outer_udp.checksum))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_packetOffsetInBits += 16;
        ...
        if (ptr->vxlan.ebpf_valid) {
            ...
            ebpf_byte = ((char *) (&ptr->vxlan.flags))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_packetOffsetInBits += 8;

            *ptr->vxlan.reserved = htonl(*ptr->vxlan.reserved << 8);
            ebpf_byte = ((char *) (&ptr->vxlan.reserved))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->vxlan.reserved))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->vxlan.reserved))[2];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 2, (ebpf_byte));
            ebpf_packetOffsetInBits += 24;

            *ptr->vxlan.vni = htonl(*ptr->vxlan.vni << 8);
            ebpf_byte = ((char *) (&ptr->vxlan.vni))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->vxlan.vni))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->vxlan.vni))[2];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 2, (ebpf_byte));
            ebpf_packetOffsetInBits += 24;

            ebpf_byte = ((char *) (&ptr->vxlan.reserved2))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_packetOffsetInBits += 8;

        }
        if (ptr->ethernet.ebpf_valid) {
            if (ebpf_packetEnd < pkt + BYTES(ebpf_packetOffsetInBits + 112)) {
                return TC_ACT_SHOT;
            }

            *ptr->ethernet.dst_addr = htonll(*ptr->ethernet.dst_addr << 16);
            ebpf_byte = ((char *) (&ptr->ethernet.dst_addr))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->ethernet.dst_addr))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->ethernet.dst_addr))[2];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 2, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->ethernet.dst_addr))[3];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 3, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->ethernet.dst_addr))[4];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 4, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->ethernet.dst_addr))[5];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 5, (ebpf_byte));
            ebpf_packetOffsetInBits += 48;

            *ptr->ethernet.src_addr = htonll(*ptr->ethernet.src_addr << 16);
            ebpf_byte = ((char *) (&ptr->ethernet.src_addr))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->ethernet.src_addr))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->ethernet.src_addr))[2];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 2, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->ethernet.src_addr))[3];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 3, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->ethernet.src_addr))[4];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 4, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->ethernet.src_addr))[5];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 5, (ebpf_byte));
            ebpf_packetOffsetInBits += 48;

            *ptr->ethernet.ether_type = bpf_htons(*ptr->ethernet.ether_type);
            ebpf_byte = ((char *) (&ptr->ethernet.ether_type))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->ethernet.ether_type))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_packetOffsetInBits += 16;

        }
        if (ptr->ipv4.ebpf_valid) {
            if (ebpf_packetEnd < pkt + BYTES(ebpf_packetOffsetInBits + 160)) {
                return TC_ACT_SHOT;
            }

            ebpf_byte = ((char *) (&ptr->ipv4.ver_ihl))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_packetOffsetInBits += 8;

            ebpf_byte = ((char *) (&ptr->ipv4.diffserv))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_packetOffsetInBits += 8;

            *ptr->ipv4.total_len = bpf_htons(*ptr->ipv4.total_len);
            ebpf_byte = ((char *) (&ptr->ipv4.total_len))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->ipv4.total_len))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_packetOffsetInBits += 16;

            *ptr->ipv4.identification = bpf_htons(*ptr->ipv4.identification);
            ebpf_byte = ((char *) (&ptr->ipv4.identification))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->ipv4.identification))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_packetOffsetInBits += 16;

            *ptr->ipv4.flags_offset = bpf_htons(*ptr->ipv4.flags_offset);
            ebpf_byte = ((char *) (&ptr->ipv4.flags_offset))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->ipv4.flags_offset))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_packetOffsetInBits += 16;

            ebpf_byte = ((char *) (&ptr->ipv4.ttl))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_packetOffsetInBits += 8;

            ebpf_byte = ((char *) (&ptr->ipv4.protocol))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_packetOffsetInBits += 8;

            *ptr->ipv4.hdr_checksum = bpf_htons(*ptr->ipv4.hdr_checksum);
            ebpf_byte = ((char *) (&ptr->ipv4.hdr_checksum))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->ipv4.hdr_checksum))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_packetOffsetInBits += 16;

            *ptr->ipv4.src_addr = htonl(*ptr->ipv4.src_addr);
            ebpf_byte = ((char *) (&ptr->ipv4.src_addr))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->ipv4.src_addr))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->ipv4.src_addr))[2];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 2, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->ipv4.src_addr))[3];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 3, (ebpf_byte));
            ebpf_packetOffsetInBits += 32;

            *ptr->ipv4.dst_addr = htonl(*ptr->ipv4.dst_addr);
            ebpf_byte = ((char *) (&ptr->ipv4.dst_addr))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->ipv4.dst_addr))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->ipv4.dst_addr))[2];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 2, (ebpf_byte));
            ebpf_byte = ((char *) (&ptr->ipv4.dst_addr))[3];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 3, (ebpf_byte));
            ebpf_packetOffsetInBits += 32;
